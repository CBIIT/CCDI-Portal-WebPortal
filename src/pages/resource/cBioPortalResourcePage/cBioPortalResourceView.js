...
import headerImg from '../../../assets/resources/CBioPortal_Header.png';
...
const CBioPortalResourceContainer = styled.div`
    width: 100%;
...
const CBioPortalResourceBody = styled.div`
    @media (min-width: 1420px) {
        width: 1420px;
    }
...
const CBioPortalResourceView = ({data}) => {
    ...
    const cbioportalContent = data.cbioportalContent;
    if (cbioportalContent) {
        sectionList.current = cbioportalContent.map((element, i) => {
    ...
    return (
        <CBioPortalResourceContainer>
            ...
            <CBioPortalResourceBody id='FederationBody'>
                <div className='navSection'>
                    <div className={stickyNavStyle} id='leftNav'>
                        <div className='navTitle'>TOPICS</div>
                        {
                            cbioportalContent && cbioportalContent.map((cbioportalItem, topicid) => {
                                const topickey = `topic_${topicid}`;
                                if (cbioportalItem.topic) {
                                    return (
                                        <div name={cbioportalItem.id} className={selectedNavTitle === cbioportalItem.id ? 'navTopicItem selected' : 'navTopicItem'} key={topickey} onClick={handleClickEvent}>{cbioportalItem.topic}</div>
                                    )
                                }
                                return (
                                    <div name={cbioportalItem.id} className={selectedNavTitle === cbioportalItem.id ? 'navTopicItem selected subtitle' : 'navTopicItem subtitle'} key={topickey} onClick={handleClickEvent}>{cbioportalItem.subtopic}</div>
                                )
                            })
                        }
                    </div>
                    ...
                    <div className='contentList'>
                        ...
                        }
                        {
                            cbioportalContent && cbioportalContent.map((cbioportalItem, mciid) => {
                                ...
                                return (
                                    <div key={cpikey}>
                                        <div id={cbioportalItem.id} className='mciTitle'>{cbioportalItem.topic && cbioportalItem.topic}</div>
                                        <div id={cbioportalItem.id} name={mciid} className='mciTitleMobile sectionCollapse' onClick={handleCollapseSection}>{cbioportalItem.topic && cbioportalItem.topic}</div>
                                        <div className="mciSection mobileCollapse" ref={sectionList.current[mciid]}>
                                            <div className='mciContentContainer'>
                                                {cbioportalItem.content && ReactHtmlParser(cbioportalItem.content)}
                                            </div>
                                            {cbioportalItem.content && <div style={{height: '40px'}} />}
                                        </div>
                                    </div>
                                )
                            })
            ...
            </CBioPortalResourceBody>
        </CBioPortalResourceContainer>
    )
}

export default CBioPortalResourceView;
