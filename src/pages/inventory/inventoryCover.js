  ...
  classes,
    ...
    const generateFacetFilters = (filters, query, queryParams) => {
        ...
        queryParams.forEach((param) => {
            if (param === 'participant_id' || param === 'u' || param === 'u_fc' || param === 'u_um' || param === 'tab') {
                    return;
            }
    ...
    useEffect(() => {
        ...
        // console.log(localFindAutocomplete);
        // console.log(initialLoading);
        if (query.size === 0 && return_2_page) {
            ...
            // let page_size_num = 10;
            // if (page_size) {
            //     page_size_num = parseInt(page_size, 10);
            //     if (isNaN(page_size_num)) page_size_num = 10;
            // }
            // store.dispatch(onRowsPerPageChange({ rowsPerPage: page_size_num, page: 0 }));
            // store.dispatch(onPageChange({ pageNumb: page_num }));
            // sorting
            // const column = query.get('sort_by') ? query.get('sort_by') : '';
            // const sort = ['desc', 'asc'].includes(query.get('sort_order')) ? query.get('sort_order') : 'desc' ;
            // store.dispatch(onColumnSort({ sort, column }));
            console.log(filters);
